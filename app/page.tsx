...
import { Lock, TrendingUp, Coins, ArrowRight, Key, CheckCircle } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden border-b border-border">
        <div className="max-w-6xl mx-auto px-4 py-20 text-center">
          ...
          <h1 className="text-5xl md:text-6xl font-bold mb-4" style={{ fontFamily: 'serif' }}>
            <span className="gold-gradient-text">Sui&apos;s Official</span>
            <br />
            <span className="gold-gradient-text">Token & Vesting Locker</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/create/lock"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-lg gold-gradient-text border border-[#D4AF37]/30 hover:bg-[#D4AF37]/10 font-semibold transition-all gold-glow"
            ...
            <Link
              href="/create/vesting"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-[#D4AF37]/10 border border-[#D4AF37]/30 hover:bg-[#D4AF37]/20 text-[#D4AF37] font-semibold transition-all"
              style={{ fontFamily: 'serif' }}
            >
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={stat.label} className="text-center">
              <div className="text-xl font-bold stat-value-gold" style={{ fontFamily: 'serif' }}>{stat.value}</div>
              <div className="text-sm text-[#D4AF37] font-medium">{stat.label}</div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        <h2 className="text-2xl font-bold mb-8 text-center" style={{ fontFamily: 'serif' }}>
          <span className="gold-gradient-text">Choose Your Tool</span>
        </h2>
        <div className="grid md:grid-cols-2 gap-6">
          {features.map((f) => (
            <Link
              key={f.title}
              href={f.href}
              className="group relative p-6 rounded-xl bg-card border border-border hover:border-[#D4AF37]/40 transition-all fade-in"
            >
              <div className={`inline-flex p-3 rounded-lg bg-secondary mb-4 ${f.glow}`}>
                <div className={f.color}>{f.icon}</div>
              </div>
              <div className="flex items-start justify-between mb-2">
                <h3 className="text-xl font-bold" style={{ fontFamily: 'serif' }}>{f.title}</h3>
              ...
              <div className="flex items-center gap-1 text-[#D4AF37] text-sm font-medium group-hover:gap-2 transition-all">
            ...
            {[
              {
                ...
                icon: <Coins className="w-5 h-5" style={{ color: '#D4AF37' }} />,
              },
              {
                step: '02',
                title: 'Create Lock or Vesting',
                desc: 'Choose your type, set the schedule, pay 1 SUI fee.',
                icon: <Key className="w-5 h-5" style={{ color: '#D4AF37' }} />,
              },
              {
                step: '03',
              ...
              <div key={item.step} className="text-center p-6">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[#D4AF37]/10 mb-4">
                  <span className="text-[#D4AF37] font-bold" style={{ fontFamily: 'serif' }}>{item.step}</span>
                </div>
                <h3 className="font-bold mb-2" style={{ fontFamily: 'serif' }}>{item.title}</h3>
      ...
      <div className="border-t border-border py-6">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground" style={{ fontFamily: 'serif' }}>SuiLock — Built on Sui</span>
          </div>
